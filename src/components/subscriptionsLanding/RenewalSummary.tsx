...
export default function TransactionsLanding(subs: DashboardSubsProps) {
  ...
  return (
    <div className="scrollbar-thumb-rounded-full scrollbar-track-rounded-full scrollbar scrollbar-thumb-lbgreen scrollbar-track-lbgreen w-1/2  py-4 rounded-lg shadow-lg bg-white max-h-100 overflow-y-auto">
      <div className="flex flex-row items-center justify-between px-8">
        ...
        <HoverCardComponent>
          ...
          <div className="flex flex-row items-center gap-4 pb-2 pt-3 bg-bggrey w-full px-6">
               ...
               <div className="px-8">
      {subs.subs.map((sub) => {
        return <TransactionDetail sub={sub} renewal={renewal} key={sub.subscriptionId} />;
      })}
      </div>
    </div>
  );
